...
async function generateAttorneyCsv() {
  const generatedAt = new Date().toISOString();

  // Summary by unique attorney across all cases
  const result = await query(`
    SELECT
      ...
      o.organization_name                 AS firm_name,
      ...
      STRING_AGG(DISTINCT c.court_id,    ' | ' ORDER BY c.court_id)    AS courts,
    ...
    WHERE ct.contact_type = 'debtor_attorney'
      AND (ct.do_not_contact = FALSE OR ct.do_not_contact IS NULL)
    GROUP BY ct.id, ct.full_name, ct.first_name, ct.last_name,
             ct.title, ct.primary_email, ct.primary_email_status,
             ct.primary_phone, ct.verification_status, o.organization_name
  ...
  logger.info(`Attorneys CSV generated: ${lines.length - 1} rows`);
  return { csv: lines.join("\n"), rowCount: lines.length - 1 };
}

module.exports = { generateAttorneyCsv };
